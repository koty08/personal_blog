...
export default function PostCategorys() {
  ...
  const handleUpdateCategory = () => {
    if (editingCategory && editingCategory.name.trim()) {
      categoryUpdate.mutate({ id: editingCategory.id, name: editingCategory.name.trim() });
    }
  };
      ...
      <PopoverContent className="w-80">
        <div className="grid gap-4">
          <div className="space-y-2">
            <h4 className="font-medium leading-none">카테고리 관리</h4>
            <p className="text-sm text-muted-foreground">카테고리를 추가, 수정, 삭제합니다.</p>
          </div>
          <Separator />
